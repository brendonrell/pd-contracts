...
| `PDProject.sol` | One per artist drop. Vanilla ERC-721 with the generative script stored fully on-chain via SSTORE2. Token hashes are deterministic and seeded with `blockhash + tokenId + minter`. |
...
- **Secondary royalties:** 5% total, split 3% artist / 2% platform via the per-Project `PaymentSplitter`.
- **Hard caps:** 10,000 Outputs per Project, 64 stickers per sheet.
- **Cooldown:** 60 days between Project deploys per artist.
